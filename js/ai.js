  ...
  async ask(userMessage) {
    try {
      ...
      if (!response.ok) {
        ...
        let msg = err.error?.message || `HTTP ${status}`;
        // Prefijo especial para que app.js muestre mensajes útiles al usuario
        if (status === 400) msg = `GEMINI_BAD_REQUEST:${msg}`;
        else if (status === 401 || status === 403) msg = `GEMINI_KEY_INVALID:${msg}`;
        else if (status === 429) msg = `GEMINI_QUOTA:${msg}`;
        else if (status >= 500) msg = `GEMINI_SERVER:${msg}`;
